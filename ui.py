...
from gbWidgets.manipulate_widget.view import ManipulateWidget
...
class MainWin(QMainWindow):
	...
	def init_manipulate_widget(self):
		...
		dock.setFeatures(QDockWidget.DockWidgetMovable)
		self.manipulatePanel = ManipulateWidget(self)
		dock.setWidget(self.manipulatePanel)
		self.addDockWidget(Qt.LeftDockWidgetArea, dock)

	def bottom_cut_text_field_changed(self):
		try:
			ld.set_preproc_bottom_cut(int(self.manipulatePanel.bottom_cut_text_field.text()))
		except:
			pass

	def top_cut_text_field_changed(self):
		try:
			ld.set_preproc_top_cut(int(self.manipulatePanel.top_cut_text_field.text()))
		except:
			pass

	def left_cut_text_field_changed(self):
		try:
			ld.set_preproc_left_cut(int(self.manipulatePanel.left_cut_text_field.text()))
		except:
			pass

	def right_cut_text_field_changed(self):
		try:
			ld.set_preproc_right_cut(int(self.manipulatePanel.right_cut_text_field.text()))
		except:
			pass
